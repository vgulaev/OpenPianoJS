import { OwnDB } from '../classes/ownDB.js';
...
const yyyyMM = date => [date.getFullYear(), mm(date.getMonth() + 1)].join('-')
...
export class PlayLog {
  ...
  regroup(items) {
    const result = {}
    items.forEach(e => {
      const dateKey = yyyyMM(e.dateTime)
      // console.log(dateKey.search("2023"))
      // if (-1 == dateKey.search("2023")) {
      //   return
      // }
      if (!result[dateKey]) {
        result[dateKey] = {
  ...
  renderItems(items) {
    return Object.entries(items).map(([name, { timeLength }]) => ({
      name, timeLength
    })).sort((a, b) => b.timeLength - a.timeLength).slice(0, 1)
    .map(e => `
      <div>${e.name}</div>
  ...
  render(items) {
    ...
    const newBody = regrouped
      .map(e => `
          ...
          <div style="border-top: 1px solid black; margin-top: 10px;">
          ${(() => {
            let mins = e.timeLength / 1000 / 60;
            let hour = Math.floor(mins / 60);
            let r = mins - hour * 60
            return hour + " h " + r.toFixed(0) + " min"
          })()}</div>
      `).join('\n')
    ...
    body.innerHTML = `
      <div style="display: grid; grid-template-columns: 1fr 3fr 1fr;">
        ...
        <div style="border-top: 1px solid black; margin-top: 10px;">${(() => {
          let total = 0
          Object.entries(regrouped).map(([name, { timeLength }]) => {
            total += timeLength;
          })
          return Math.floor(total / 1000 / 60 / 60) + " hours"
        })()}</div>
      </div>`
  }
}
